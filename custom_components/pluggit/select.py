...
from .device_map import SELECTS, PluggitSelectEntityDescription
...
async def async_setup_entry(hass: HomeAssistant, config_entry, async_add_entities):
    ...
    for description in SELECTS:
        if await device.async_install_entity(description):
            select = PluggitSelect(device, description)
...
class PluggitSelect(SelectEntity, PluggitEntity):
    """Pluggit select."""

    def __init__(
        self,
        device: Device,
        description: PluggitSelectEntityDescription,
    ) -> None:
        """Init select."""
        super().__init__(device)
        self._device = device
        self._attr_has_entity_name = True
        self.entity_description: PluggitSelectEntityDescription = description
